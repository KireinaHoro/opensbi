...
double bench(Int N, Int iters) {
    // Allocate all the memory continuously so we aren't affected by the
    // particulars of the system allocator:
    ...
    // This is the worst case scenario for a linked list, which is exactly what
    // we want to measure. Without the random_shuffle we get O(N) because it
    // enables the pre-fetcher to do its job. Without a prefetcher, a random
    // memory access in N bytes of RAM costs O(N^0.5) due to the Bekenstein
    // bound. This means we get O(N^1.5) complexity of a linked list traversal.
    ...
    Int dur = r_time() - start;
    double ns = 10 * dur; // 100MHz
...
void ram_bench() {
    ...
    Int stopsPerFactor =
        4; // For every power of 2, how many measurements do we do?
    Int minElemensFactor = 6; // First measurement is 2^this number of elements.
    Int maxElemsFactor = 30;  // Last measurement is 2^this number of elements.
                              // 30 == 16GB of memory
    // Int elemsPerMeasure = Int(1) << 28; // measure enough times to process
    // this many elements (to get a good average)
