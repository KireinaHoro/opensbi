...
#define XCUSTOM(x, rd, rs1, rs2, funct)                                        \
  XCUSTOM_OPCODE(x) | (rd << (7)) | (0x3 << (7 + 5)) |                         \
      ((rd != 0) & 1 << (7 + 5 + 2)) | (rs1 << (7 + 5 + 3)) |                  \
      (rs2 << (7 + 5 + 3 + 5)) | (EXTRACT(funct, 7, 0) << (7 + 5 + 3 + 5 + 5))
...
#define ROCC_INSTRUCTION_R_R_R(x, rd, rs1, rs2, funct, rd_n, rs1_n, rs2_n)     \
  {                                                                            \
    ...
    asm volatile(".word " STR(XCUSTOM(x, rd_n, rs1_n, rs2_n, funct)) "\n\t"    \
                 : "=r"(rd_)                                                   \
...
#define ROCC_INSTRUCTION_0_R_R(x, rs1, rs2, funct, rs1_n, rs2_n)               \
  {                                                                            \
    register uint64_t rs1_ asm("x" #rs1_n) = (uint64_t)rs1;                    \
    register uint64_t rs2_ asm("x" #rs2_n) = (uint64_t)rs2;                    \
    asm volatile(".word " STR(XCUSTOM(x, 0, rs1_n, rs2_n,                      \
                                      funct)) "\n\t" ::[_rs1] "r"(rs1_),       \
                 [ _rs2 ] "r"(rs2_));                                          \
  }
