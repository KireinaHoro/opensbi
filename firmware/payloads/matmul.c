	...
	for (int activation = 0; activation <= 2; ++activation) {
		for (int shift = 0; shift <= 4; shift += 4) {
			...
			for (size_t g = 0; g < N * N * N; ++g) {
				...
				if (accumulate[g])
					matadd(gold_full[g], gold_full[g - 1],
					       gold_full[g]);
			}
			...
			printf("Setting mode\n");
			gemmini_config_ex(WEIGHT_STATIONARY, activation, 0,
					  shift, relu6_shift);
