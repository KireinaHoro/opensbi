...
void matshift(int64_t full[DIM][DIM], elem_t out[DIM][DIM], int shift)
{
	for (size_t r = 0; r < DIM; r++)
		for (size_t c = 0; c < DIM; c++) {
			...
			int64_t shifted =
				ROUNDING_RIGHT_SHIFT(full[r][c], shift);

			// Saturate and cast element
			int64_t elem =
				shifted > elem_t_max
					? elem_t_max
					: (shifted < elem_t_min ? elem_t_min
								: shifted);
			out[r][c] = elem;
		}
}
...
// Tiling functions
static void sp_tiled_matmul_os(const elem_t *A, const elem_t *B, const acc_t *D,
			       ...
			       bool repeating_bias)
{
	...
	if (D != NULL && !no_bias) {
		const size_t D_stride =
			repeating_bias ? 0 : D_row_len * sizeof(acc_t);
		...
		for (size_t i = 0; i < I; i++) {
			for (size_t j = 0; j < J; j += D_blocks) {
				...
				const uint32_t D_sp_addr_acc =
					D_sp_addr_start + (i * J + j) * DIM;

				const size_t blocks =
					j + D_blocks <= J ? D_blocks : J - j;

				const size_t cols =
					blocks * DIM - (j == J - 1 ? pad_J : 0);
				const size_t rows =
					DIM - (i == I - 1 ? pad_I : 0);

				gemmini_extended_mvin(
					D_dram_addr, D_sp_addr_acc, cols, rows);
			}
		}
	}
	...
	for (size_t j = 0; j < J; j += B_blocks) {
		for (size_t k = 0; k < K; k++) {
			const elem_t *const B_dram_addr =
				B + (k * B_row_len + j) * DIM;
			const uint32_t B_sp_addr =
				B_sp_addr_start + (k * J + j) * DIM;
			const size_t blocks =
				j + B_blocks <= J ? B_blocks : J - j;
			const size_t cols =
				blocks * DIM - (j == J - 1 ? pad_J : 0);
			const size_t rows = DIM - (k == K - 1 ? pad_K : 0);
			gemmini_extended_mvin(B_dram_addr, B_sp_addr, cols,
					      rows);
		}
	}
	...
	for (size_t i = 0; i < I; i++) {
		for (size_t k = 0; k < K; k += A_blocks) {
			const elem_t *const A_dram_addr =
				A + (i * A_row_len + k) * DIM;
			const uint32_t A_sp_addr =
				A_sp_addr_start + (i * K + k) * DIM;
			const size_t blocks =
				k + A_blocks <= K ? A_blocks : K - k;
			const size_t cols =
				blocks * DIM - (k == K - 1 ? pad_K : 0);
			const size_t rows = DIM - (i == I - 1 ? pad_I : 0);
			gemmini_extended_mvin(A_dram_addr, A_sp_addr, cols,
					      rows);
		}
	}
	...
	for (size_t i = 0; i < I; i++) {
		for (size_t j = 0; j < J; j++) {
			...
			for (size_t k = 0; k < K; k++) {
				...
				uint32_t out_sp_addr =
					k == K - 1 ? C_sp_addr : GARBAGE_ADDR;

				// If we're not using a bias, then we want to overwrite what's in the
				// accumulator, rather than writing over it
				int no_bias_new_matrix =
					no_bias && D != NULL && k == K - 1;
				...
				gemmini_extended_preload(GARBAGE_ADDR,
							 out_sp_addr, DIM, DIM,
							 C_cols, C_rows);

				if (k == 0) { // First iteration
					gemmini_extended_compute_preloaded(
						A_sp_addr, B_sp_addr, A_cols,
	...
	if (C != NULL) {
		for (size_t i = 0; i < I; i++) {
			for (size_t j = 0; j < J; j++) {
				...
				gemmini_extended_mvout(C_dram_addr, C_sp_addr,
						       C_cols, C_rows);
			}
		}
	}
...
static void sp_tiled_matmul_ws(const elem_t *A, const elem_t *B, const acc_t *D,
			       elem_t *C, size_t I, size_t J, size_t K,
			       size_t pad_I, size_t pad_J, size_t pad_K,
			       size_t A_row_len, size_t B_row_len,
			       size_t D_row_len, size_t C_row_len, bool no_bias,
	...
	for (size_t j = 0; j < J; j++) {
		for (size_t k = 0; k < K; k++) {
			...
			for (size_t i = 0; i < I; i++) {
				...
				uint32_t pre_sp_addr =
					i == 0 ? B_sp_addr : GARBAGE_ADDR;
				...
				gemmini_extended_preload(
					pre_sp_addr, out_sp_addr, B_cols,
					B_rows, C_cols, C_rows);

				if (i == 0) { // First iteration
					gemmini_extended_compute_preloaded(
						A_sp_addr, GARBAGE_ADDR, A_cols,
						A_rows, DIM, DIM);
				} else { // All other iterations
					gemmini_extended_compute_accumulated(
						A_sp_addr, GARBAGE_ADDR, A_cols,
			...
			for (size_t k0 = 0; k0 < K0; k0++) {
				...
				if (dataflow == OUTPUT_STATIONARY) {
					sp_tiled_matmul_os(
						...
						&B[k0 * tile_K * DIM]
						  [j0 * tile_J * DIM],
						pre, out, I, J, K, pad_I, pad_J,
						pad_K, dim_K, dim_J, dim_J,
						dim_J, no_bias, repeating_bias);
				} else {
					sp_tiled_matmul_ws(
						&A[i0 * tile_I * DIM]
						  [k0 * tile_K * DIM],
		       ...
		       size_t relu6_shift, bool repeating_bias)
{
	...
	for (size_t i = 0; i < dim_I; i++) {
		for (size_t j = 0; j < dim_J; j++) {
			...
			acc_t result =
				no_bias ? 0 : ((acc_t(*)[dim_J])D)[bias_row][j];
		  ...
		  enum tiled_matmul_type_t tiled_matmul_type)
{
	...
	if (tiled_matmul_type == OS || tiled_matmul_type == WS) {
		tiled_matmul_outer(dim_I, dim_J, dim_K, A, B, D, C, tile_I,
				   tile_J, tile_K, act, shift, relu6_shift,
				   repeating_bias, (int)tiled_matmul_type);
	} else /*if (tiled_matmul_type == CPU)*/ {
		matmul_cpu(dim_I, dim_J, dim_K, A, B, D, C, act, shift,
			   relu6_shift, repeating_bias);
	}
}
...
#define max_tile_k (mats_in_partition / max_tile_i_j)
	...
	const size_t tile_I = dim_I_padded / DIM < max_tile_i_j
				      ? dim_I_padded / DIM
				      : max_tile_i_j;
	const size_t tile_J = dim_J_padded / DIM < max_tile_i_j
				      ? dim_J_padded / DIM
				      : max_tile_i_j;
	const size_t tile_K = dim_K_padded / DIM < max_tile_k
				      ? dim_K_padded / DIM
				      : max_tile_k;

	tiled_matmul(dim_I, dim_J, dim_K, A, B, D, C, act, shift, relu6_shift,
		     repeating_bias, tile_I, tile_J, tile_K, tiled_matmul_type);
